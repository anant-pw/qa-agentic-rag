...
def build_real_context(question: str, pg_host, pg_port, pg_db, pg_user, pg_password,
                       os_host, os_port, ollama_host, ollama_port, alias,
                       doc_type=None, module=None, status=None) -> tuple[list[dict], list[str]]:
    ...
    doc_type/module/status added after a real diagnostic mistake: the
    first version of this script never passed structured filters to
    hybrid_search() at all, so a structured_filter comparison run (e.g.
    "How many bugs are Open in Login") silently ran UNFILTERED retrieval
    -- both Ollama and Groq answered from whatever 5 documents matched
    the raw question text semantically, which for that query was mostly
    wrong (1 of 5 retrieved docs actually matched Login+Open). Both
    models agreeing on an answer is not evidence the answer is right if
    the input they were both given was already wrong -- confirmed by
    comparing against live Postgres ground truth (4 Login+Open bugs, not
    1). This is the same class of bug eval/run_eval_generation.py's
    original call_generate() had, fixed the same way: forward the actual
    filters through, don't rely on the raw question text alone."""
    ...
    hits = hybrid_search(
        ...
        doc_type=doc_type, module=module, status=status,
    )
...
def call_groq(messages: list[dict]) -> str:
    ...
    try:
        ...
        logger.info(
            "groq_chat_stats: model=%s total_time_sec=%.3f",
            GROQ_MODEL,
            total_time,
        )
...
def call_ollama(
    ...
    """Calls Ollama /api/chat via stream_chat and logs:
      - time_to_first_token_sec
      - total_generation_time_sec
      - total_tokens (eval_count from the final done chunk, if present)

    think: passed straight through to stream_chat's new "think" param --
    see app/generation/llm.py's docstring for why this exists (qwen3.5:9b's
    500-670s runs turned out to be dominated by an unrequested reasoning
    trace, not attention cost, once total_tokens was compared against the
    ...
    try:
        for content, maybe_done_chunk in stream_chat(
            ...
            model=model,
            think=think,
            log_stats=False,  # we log here instead
        ):
            if first_token_time is None:
                first_token_time = time.perf_counter()
            fragments.append(content)

            if maybe_done_chunk and maybe_done_chunk.get("done"):
                total_tokens = maybe_done_chunk.get("eval_count")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--doc-type", default=None, help="Filter, e.g. bug_report")
    parser.add_argument("--module", default=None, help="Filter, e.g. Login")
    parser.add_argument("--status", default=None, help="Filter, e.g. Open")
    parser.add_argument("--think", default=None, choices=["true", "false"],
                         help="Pass Ollama's 'think' param for hybrid-reasoning models "
                              "(qwen3.5, deepseek-r1, etc.). Omit to use the model's default. "
                              "Try --think false on qwen3.5:9b if it's unexpectedly slow.")
    ...
    context_blocks, doc_ids = build_real_context(
        ...
        doc_type=args.doc_type, module=args.module, status=args.status,
    )
    ...
    print("=" * 60)
    think_arg = None if args.think is None else (args.think == "true")
    print(call_ollama(messages, args.ollama_host, args.ollama_port, args.ollama_model, think=think_arg))
